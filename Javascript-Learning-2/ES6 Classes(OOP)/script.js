...
// ES6 classes is another way of writing the constructor function,it allows us to do the exact same thing but with more simpler syntex .While taking ES6 classes we'll use "class" keyword and then after that name of class which is going to be in capital letter and in our case it is "Sports","Cl" will also come after the name of class(Sports).Here we'll use "constructor" keyword for taking the constructor function.

class SportsCl {
  constructor(firstSport, secondSport) {
    this.firstSport = firstSport;
    this.secondSport = secondSport;
  }
  testGame() {
    console.log(`Qas likes to play ${this.firstSport}`);
  }
}
...
// Here "SportsCl" is just like constructor function and the only difference is that this kinda syntex is more easy and clean as compared to the constructor function's syntex .

//It's the personal preference to use "constructor-function" or the "classes" in your code .
